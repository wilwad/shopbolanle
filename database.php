<?php
	...
	$database->query(settings::sql_create_db) or die('sql_create_db: ' . $database->error);		
 ...
 */
if (!function_exists("get_default_view")){
	...
	if (!$ret || !$ret->num_rows)
	{
		...
		while ($row = $ret->fetch_array())
		{
			...
			switch (strtolower($field)){
                ...
                case 'package_cycle_id':
                     $sql = "SELECT * 
                             FROM `list_package_cycles`;";
				...
				case 'dob':
					...
					$inputs .=  "<div class='form-group'>
							...
							for='$field'>$field_ $star_required</label>
				...
				case 'assistance_required_id':
					$sql = "SELECT * 
					        FROM `list_assistance_type`
					        ;";
					$select = build_selectbox($sql,$field,$val);
				...
				case 'born_in_namibia_qualifying_criteria_id':
					$sql = "SELECT * 
					        FROM `list_qualifying_criteria`
					        ;";
					$select = build_selectbox($sql,$field,$val);
				...
				case 'highest_qualification_id':
					$sql = "SELECT * 
					        FROM `list_highest_qualification`
					        ;";
					$select = build_selectbox($sql,$field,$val);
				...
				case 'verification_outcome_id':
					$sql = "SELECT * 
					        FROM `list_verification_outcome`
					        ;";
					$select = build_selectbox($sql,$field,$val);
          ...
          $input = "<div class='input-group'>
				...
				case 'ssc_number':
						$field_ = "Social Security Number";
		          $input = "
					            <input type='text' placeholder='Social Security number' class='form-control' 
					                                 name='$field' id='$field' value='$val'>";
